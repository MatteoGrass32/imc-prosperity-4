...
class MarketMakingStrategy(Strategy):
    def run(self) -> List[Order]:
       ...
       # BACK TO BASICS SIZE (STABLE)
        ...
        orders: List[Order] = []
        if buy_qty > 0: orders.append(Order(self.product, bid_price, buy_qty))
        if sell_qty > 0: orders.append(Order(self.product, ask_price, -sell_qty))
        return orders
