...
%   Basato sui risultati MC (50 run x 500k paths, CI 90%)
% =========================================================

% --- DATI DAL PRICER (IC al 90%) ---
names   = {'AC_50_P';'AC_50_C';'AC_35_P';'AC_40_P';'AC_45_P';'AC_60_C'; ...
           'AC_50_P_2';'AC_50_C_2';'AC_50_CO';'AC_40_BP';'AC_45_KO'};

theos   = [12.0268; 12.0275; 4.3361; 6.5096; 9.0889; 8.7924;
            9.8703;  9.8691; 21.8957; 4.7683; 0.2065];
ci_low  = [12.0252; 12.0201; 4.3344; 6.5077; 9.0870; 8.7855;
            9.8684;  9.8633; 21.8889; 4.7678; 0.2062];
ci_high = [12.0284; 12.0350; 4.3378; 6.5115; 9.0907; 8.7994;
            9.8721;  9.8749; 21.9026; 4.7688; 0.2068];
...
p_put         = 1 - p_call;
delta_call_rem = normcdf((log(S0/50) + 0.5*sigma^2*T1w) / (sigma*sqrt(T1w)));
delta_put_rem  = delta_call_rem - 1;
deltas(9)     = p_call * delta_call_rem + p_put * delta_put_rem;
...
Su = [(S0+dS_fd)*ones(1,paths_delta); (S0+dS_fd)*cumprod(ret_d,1)];
Sd = [(S0-dS_fd)*ones(1,paths_delta); (S0-dS_fd)*cumprod(ret_d,1)];
surv_u = min(Su,[],1) > 35;
surv_d = min(Sd,[],1) > 35;
deltas(11) = mean(surv_u.*max(45-Su(end,:),0) - surv_d.*max(45-Sd(end,:),0)) / (2*dS_fd);
...
for i = 1:length(names)
    if actions(i) == "Buy"
        cash_flow = cash_flow - asks(i) * volumes(i);
    elseif actions(i) == "Sell"
        cash_flow = cash_flow + bids(i) * volumes(i);
    end
end
if ac_action == "Sell"
    cash_flow = cash_flow + ac_bid * ac_volume;
elseif ac_action == "Buy"
    cash_flow = cash_flow - ac_ask * ac_volume;
end
...
fprintf('\n%s\n', sep);
fprintf('  ALLOCAZIONE OTTIMA  |  Filtro: CI 90%%  |  Contract size: %d\n', contract_size);
fprintf('%s\n', sep);
...
fprintf('%s\n', sep2);
fprintf('  Cash flow iniziale (UI)      : %+.2f XIRECs  (ignorabile)\n', cash_flow);
