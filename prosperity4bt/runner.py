...
def enforce_limits(
    ...
    for product in data.products:
        ...
        total_short = sum(abs(order.quantity)
                          for order in product_orders if order.quantity < 0)

        if product_position + total_long > lim or product_position - total_short < -lim:
            sandbox_log_lines.append(
                f"Orders for product {product} exceeded limit of {lim} set")
            orders.pop(product)
...
def match_buy_order(
    ...
    for price in price_matches:
        ...
        volume = min(order.quantity, abs(
            order_depth.sell_orders[price]), max_buy)
        if volume <= 0:
            continue

        trades.append(Trade(order.symbol, price, volume,
                      "SUBMISSION", "", state.timestamp))

        state.position[order.symbol] = state.position.get(
            order.symbol, 0) + volume
    ...
    for market_trade in market_trades:
        ...
        trades.append(
            Trade(order.symbol, order.price, volume, "SUBMISSION",
                  market_trade.trade.seller, state.timestamp)
        )
...
def match_sell_order(
    ...
    order_depth = state.order_depths[order.symbol]
    price_matches = sorted((price for price in order_depth.buy_orders.keys(
    ) if price >= order.price), reverse=True)
    for price in price_matches:
        lim = get_position_limit(order.symbol, limits_override)
        pos = state.position.get(order.symbol, 0)
        max_sell = max(0, pos + lim)
        volume = min(abs(order.quantity),
                     order_depth.buy_orders[price], max_sell)
        if volume <= 0:
            continue
    ...
    for market_trade in market_trades:
        ...
        trades.append(Trade(order.symbol, order.price, volume,
                      market_trade.trade.buyer, "SUBMISSION", state.timestamp))
...
def match_orders(
    ...
    for product, trades in market_trades.items():
        ...
        remaining_market_trades = [
            t.trade for t in trades if t.trade.quantity > 0]

        state.market_trades[product] = remaining_market_trades
        result.trades.extend([TradeRow(trade)
...
def run_backtest(
    ...
    timestamps_iterator = tqdm(
        timestamps, ascii=True) if show_progress_bar else timestamps
    ...
    for timestamp in timestamps_iterator:
        ...
        match_orders(state, data, orders, result,
                     trade_matching_mode, limits_override)
